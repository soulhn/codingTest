...
function rockPaperScissors() {
  ...
  const arrRsp = ["rock", "paper", "scissors"];
  for (let i = 0; i < arrRsp.length; i++) {
    for (let j = 0; j < arrRsp.length; j++) {
      for (let k = 0; k < arrRsp.length; k++) {
        const newArr = [arrRsp[i], arrRsp[j], arrRsp[k]];
        result.push(newArr);
      }
    }
  }
